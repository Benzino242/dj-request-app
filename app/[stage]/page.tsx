...
const paymentSuccessTranslations: Record<Language,
PaymentSuccessTranslation> = {
en: {
...
savedSupport: "Your request is saved. Keep this reference if you need
support.",
beforePaymentReference: "After payment, you’ll receive a Paystack
...
savedSupport:
"リクエストは保存されました。サポートが必要な場合はこの参照番号を保管してください。",
beforePaymentReference:
"支払い後、サポート用のPaystack参照番号を受け取ります。",
},
ko: {
...
savedSupport: "요청이 저장되었습니다. 지원이 필요하면 이 참조번호를
보관하세요.",
beforePaymentReference: "결제 후 지원용 Paystack 참조번호를 받게
됩니다.",
},
id: {
...
savedSupport: "Request Anda tersimpan. Simpan referensi ini jika perlu
bantuan.",
beforePaymentReference: "Setelah pembayaran, Anda akan menerima
referensi Paystack untuk bantuan.",
},
ms: {
...
savedSupport: "Permintaan anda telah disimpan. Simpan rujukan ini jika
perlukan bantuan.",
beforePaymentReference: "Selepas bayaran, anda akan menerima rujukan
Paystack untuk bantuan.",
},
th: {
...
beforePaymentReference: "หลังชำระเงิน คุณจะได้รับรหัสอ้างอิง Paystack
สำหรับการช่วยเหลือ",
},
hi: {
...
beforePaymentReference: "Payment के बाद आपको support के लिए Paystack
reference मिलेगा.",
},
ar: {
...
savedSupport: "Yêu cầu của bạn đã được lưu. Hãy giữ mã này nếu cần hỗ
trợ.",
beforePaymentReference: "Sau khi thanh toán, bạn sẽ nhận mã Paystack để
được hỗ trợ.",
},
tl: {
...
savedSupport: "Naka-save na ang request mo. Itago ang reference na ito
kung kailangan ng support.",
beforePaymentReference: "Pagkatapos magbayad, makakatanggap ka ng
Paystack reference para sa support.",
},
pt: {
...
savedSupport: "O seu pedido foi guardado. Guarde esta referência se
precisar de suporte.",
beforePaymentReference: "Após o pagamento, receberá uma referência
Paystack para suporte.",
},
es: {
...
savedSupport: "Tu solicitud está guardada. Conserva esta referencia si
necesitas soporte.",
beforePaymentReference: "Después del pago, recibirás una referencia de
Paystack para soporte.",
},
fr: {
paymentReceived: "Paiement reçu",
requestSent: "Demande envoyée au DJ",
boostAdded: "Votre boost a été ajouté à la demande existante.",
requestAdded: "Votre demande de chanson a été ajoutée à la file en
direct.",
song: "Chanson",
amount: "Montant",
paystackReference: "Référence Paystack",
done: "Terminé",
savedSupport: "Votre demande est enregistrée. Gardez cette référence si
vous avez besoin d’aide.",
beforePaymentReference: "Après le paiement, vous recevrez une référence
Paystack pour le support.",
},
de: {
...
requestAdded: "Dein Songwunsch wurde zur Live-Warteschlange
hinzugefügt.",
song: "Song",
amount: "Betrag",
paystackReference: "Paystack-Referenz",
done: "Fertig",
savedSupport: "Deine Anfrage wurde gespeichert. Bewahre diese Referenz
für den Support auf.",
beforePaymentReference: "Nach der Zahlung erhältst du eine
Paystack-Referenz für den Support.",
},
ru: {
...
savedSupport: "Ваш запрос сохранён. Сохраните эту ссылку для
поддержки.",
beforePaymentReference: "После оплаты вы получите ссылку Paystack для
поддержки.",
},
tr: {
...
savedSupport: "İsteğin kaydedildi. Destek gerekirse bu referansı
sakla.",
beforePaymentReference: "Ödemeden sonra destek için bir Paystack
referansı alacaksın.",
},
it: {
...
savedSupport: "La tua richiesta è salvata. Conserva questo riferimento
per il supporto.",
beforePaymentReference: "Dopo il pagamento riceverai un riferimento
Paystack per il supporto.",
},
nl: {
...
savedSupport: "Je verzoek is opgeslagen. Bewaar deze referentie voor
support.",
beforePaymentReference: "Na betaling ontvang je een Paystack-referentie
voor support.",
},
pl: {
...
savedSupport: "Twoja prośba została zapisana. Zachowaj tę referencję w
razie potrzeby wsparcia.",
beforePaymentReference: "Po płatności otrzymasz referencję Paystack do
wsparcia.",
},
el: {
...
savedSupport: "Το αίτημά σας αποθηκεύτηκε. Κρατήστε αυτή την αναφορά για
υποστήριξη.",
beforePaymentReference: "Μετά την πληρωμή, θα λάβετε αναφορά Paystack
για υποστήριξη.",
},
uk: {
...
savedSupport: "Ваш запит збережено. Збережіть цей референс для
підтримки.",
beforePaymentReference: "Після оплати ви отримаєте референс Paystack для
підтримки.",
},
};
...
const nowPlayingCountdownTranslations: Record<Language,
NowPlayingCountdownTranslation> = {
...
const [duplicateRequest, setDuplicateRequest] = useState<Request |
null>(null);
...
nowPlayingCountdownTranslations[language] ||
nowPlayingCountdownTranslations.en;
...
const requestsClosedTitle = isRejectedDJ ? "Requests locked" :
t.requestsClosed;
const requestsClosedMessage = isRejectedDJ
? "This DJ is not accepting song requests right now. Please check back
later."
...
return Math.max(0, NOW_PLAYING_DURATION_MS - (Date.now() -
playedAtTime));
}
function formatNowPlayingRemaining(request: Request) {
const totalSeconds = Math.ceil(getNowPlayingRemainingMs(request) /
1000);
...
alert(
"Payment succeeded, but no payment reference was returned. Please
...
alert(
`Payment could not be verified and saved yet. Please contact Blackline
support with this reference: ${paymentReference}`
);
return;
}
...
alert(
`Payment succeeded, but something went wrong while verifying and saving
your request. Please contact Blackline support with this reference:
${paymentReference}`
);
} finally {
setSubmitting(false);
...
>
{flashAlert && (
<div className="fixed inset-0 bg-purple-600/30 z-50 pointer-events-none
animate-pulse" />
)}
{nowPlaying && (
...
shadow-[0_0_40px_rgba(168,85,247,0.7)] border border-purple-300
text-center"
style={{
...
<div className="inline-flex items-center gap-2 mt-4 bg-black/40 border
border-white/20 text-white px-4 py-2 rounded-full text-xs font-bold">
<span>⏳</span>
<span>{nowPlayingCountdownText.expiresIn}
{formatNowPlayingRemaining(nowPlaying)}</span>
</div>
</div>
</div>
</div>
...
from-green-950 via-zinc-900 to-black border border-green-500/60
rounded-3xl p-6 text-center shadow-[0_0_35px_rgba(34,197,94,0.25)]">
<div className="absolute -top-12 -right-12 w-32 h-32 bg-green-500/20
rounded-full blur-3xl" />
<div className="absolute -bottom-12 -left-12 w-32 h-32 bg-purple-500/20
rounded-full blur-3xl" />
<div className="relative z-10">
<div className="w-16 h-16 mx-auto rounded-full bg-green-500/20 border
border-green-400/40 flex items-center justify-center text-3xl mb-4">
✅
</div>
...
<div className="bg-black/40 border border-zinc-800 rounded-2xl p-4 mt-5
text-left">
...
className="mt-5 w-full bg-purple-600 hover:bg-purple-700 px-5 py-3
...
<h1 className="mx-auto mb-3 max-w-full px-2 text-center
text-[clamp(2.25rem,10vw,4.5rem)] font-black uppercase leading-[0.95]
tracking-wide text-purple-400
drop-shadow-[0_0_18px_rgba(168,85,247,0.45)] break-words
...
hover:border-purple-500/50 hover:text-purple-300 transition"
>
�� {dj.venue}
</a>
)}
...
target="_blank"
rel="noopener noreferrer"
className="inline-block bg-purple-500/15 border border-purple-500/30
text-purple-300 text-sm font-bold px-4 py-2 rounded-full
...
{!canAcceptRequests && (
<div className="relative overflow-hidden bg-red-950 border
border-red-700 p-5 rounded-3xl mb-6 text-center
shadow-[0_0_25px_rgba(239,68,68,0.18)]">
<div className="absolute -top-10 -right-10 w-28 h-28 bg-red-500/20
rounded-full blur-2xl" />
<div className="relative z-10">
<div className="w-14 h-14 mx-auto rounded-full bg-red-500/20 border
border-red-400/40 flex items-center justify-center text-2xl mb-4">
��
</div>
<p className="text-red-200 text-xs font-black uppercase tracking-[0.2em]
mb-2">
{requestsClosedTitle}
</p>
...
<div className="relative overflow-hidden bg-gradient-to-br
from-purple-950 via-purple-900 to-black border border-purple-500/60 p-5
rounded-3xl mb-6 shadow-[0_0_30px_rgba(168,85,247,0.22)]">
<div className="absolute -top-10 -right-10 w-28 h-28 bg-purple-500/20
rounded-full blur-2xl" />
<div className="relative z-10">
<p className="text-purple-200 text-xs font-black uppercase
tracking-[0.2em] mb-2">
�� {t.skipTheQueue}
</p>
...
<div className="absolute z-50 w-full mt-2 bg-zinc-900 border
border-zinc-700 rounded-xl overflow-hidden shadow-xl max-h-80
overflow-y-auto">
{songResults.map((track) => (
<button
key={track.id}
type="button"
className="w-full text-left p-3 hover:bg-zinc-800 transition border-b
border-zinc-800"
onClick={() => {
setSong(track.song);
...
<button
type="button"
className="w-full text-left p-3 bg-black hover:bg-zinc-800 transition
...
{duplicateRequest && (
<div className="relative overflow-hidden bg-gradient-to-br
from-yellow-950 via-orange-950 to-black border border-yellow-500/70 p-4
rounded-3xl mt-4 shadow-[0_0_35px_rgba(234,179,8,0.18)]">
<div className="absolute -top-12 -right-12 w-32 h-32 bg-yellow-500/20
rounded-full blur-3xl" />
<div className="relative z-10">
...
<div className="bg-black/30 border border-yellow-500/10 rounded-2xl
p-3">
<p className="text-yellow-300 text-xs font-black uppercase
tracking-wide">
{t.currentPosition}
</p>
...
key={amount}
type="button"
className="bg-purple-600 hover:bg-purple-700 rounded-xl py-3 font-bold
...
<div className="absolute z-50 w-full mt-2 bg-zinc-900 border
border-zinc-700 rounded-xl overflow-hidden shadow-xl max-h-96
overflow-y-auto">
<div className="p-3 text-xs text-purple-300 font-bold bg-black border-b
border-zinc-800">
...
<div className="bg-black border border-purple-800 rounded-2xl p-5 mt-4
...
disabled={!canAcceptRequests}
className="bg-purple-700 hover:bg-purple-600 px-3 py-3 rounded-xl
font-bold text-sm disabled:opacity-40 transition-all hover:scale-105"
>
+{tipCurrency}
...
className="w-full bg-purple-600 hover:bg-purple-700 transition p-4
rounded-xl text-xl font-semibold disabled:opacity-50"
>
{!canAcceptRequests
? t.requestsClosed
: tipCurrency !== "GHS"
...
className="w-full mt-3 bg-zinc-800 hover:bg-zinc-700 transition p-4
rounded-xl text-xl font-semibold border border-zinc-700"
>
�� Book DJ
</button>
)}
...
<div className="bg-zinc-900 border border-cyan-500 p-5 rounded-3xl
text-center shadow-[0_0_25px_rgba(34,211,238,0.3)] mb-6">
{upNext.artwork && (
<img
src={upNext.artwork}
alt={upNext.song}
...
shadow-[0_0_25px_rgba(250,204,21,0.45)] scale-[1.02]"
: index === 0
...
>
{isVIPRequest(request.tip_amount) && (
<div className="mb-2 inline-block bg-yellow-500 text-black text-xs
font-black px-3 py-1 rounded-full">
{t.vipRequest}
</div>
)}
