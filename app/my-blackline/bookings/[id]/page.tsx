...
export default function BookingEventWorkspacePage() {
  ...
  const [newMusic, setNewMusic] = useState<Omit<MusicItem, "id">>({ category: "must_play", title: "", artist: "", notes: "" });
  ...
  function addMusicItem() { if (!newMusic.title.trim()) return; setPlan((current) => ({ ...current, music_requests: [...current.music_requests, { ...newMusic, id: makeId(), title: newMusic.title.trim(), artist: newMusic.artist.trim(), notes: newMusic.notes.trim() }] })); setNewMusic({ category: "must_play", title: "", artist: "", notes: "" }); }
  ...
  return <main dir={language === "ar" ? "rtl" : "ltr"} className="min-h-screen bg-black px-4 py-6 text-white md:px-8 md:py-10">
    ...
    <div className="relative mx-auto max-w-6xl">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5 md:p-7"><h2 className="text-2xl font-black">🎵 {text.musicPlan}</h2><div className="mt-5 space-y-4">{(Object.keys(categories) as (keyof typeof categories)[]).map((category) => <div key={category} className={`rounded-2xl border bg-black p-4 ${categories[category].color}`}><h3 className="font-black">{categories[category].icon} {categories[category].title}</h3><div className="mt-3 space-y-2">{!musicGroups[category].length && <p className="text-sm text-zinc-600">{text.nothingAdded}</p>}{musicGroups[category].map((item) => <div key={item.id} className="flex gap-2 rounded-xl bg-zinc-950 p-3"><div className="flex-1"><p className="font-bold">{item.title}{item.artist ? ` — ${item.artist}` : ""}</p>{item.notes && <p className="mt-1 text-xs text-zinc-500">{item.notes}</p>}</div>{canEdit && <button onClick={() => setPlan((current) => ({ ...current, music_requests: current.music_requests.filter((entry) => entry.id !== item.id) }))}>✕</button>}</div>)}</div></div>)}</div>{canEdit && <div className="mt-5 grid gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4"><select value={newMusic.category} onChange={(event) => setNewMusic((current) => ({ ...current, category: event.target.value as MusicItem["category"] }))} className={fieldClass}><option value="must_play">{text.mustPlay}</option><option value="do_not_play">{text.doNotPlay}</option><option value="special_moment">{text.specialMoments}</option></select><input value={newMusic.title} onChange={(event) => setNewMusic((current) => ({ ...current, title: event.target.value }))} placeholder={text.songTitle} className={fieldClass}/><input value={newMusic.artist} onChange={(event) => setNewMusic((current) => ({ ...current, artist: event.target.value }))} placeholder={text.artist} className={fieldClass}/><input value={newMusic.notes} onChange={(event) => setNewMusic((current) => ({ ...current, notes: event.target.value }))} placeholder={text.optionalNotes} className={fieldClass}/><button onClick={addMusicItem} className="rounded-xl bg-purple-600 p-3 font-black">{text.addSong}</button></div>}</section>
      </div>
